...
### 一面
#### 算法题
1、创建一个函数实现tail -n，返回最后n行数据
2、消消乐，给一个字符串，如果相邻的两个数字加起来等于10，就可以消掉，返回最后剩余的字符串长度
	比如：213792，37可以消掉，然后19可以消掉，最后剩余 22，返回2
#### 问项目
#### TCP连接
1、服务端挂掉重启之后，客户端会收到什么通知吗
```
在TCP连接中，如果服务端意外挂掉并重新启动，客户端通常会收到一个连接断开的通知。这通知是通过TCP协议中的一系列机制来完成的，而具体表现取决于操作系统和使用的编程语言。

连接重置（Connection Reset）： 当服务端挂掉并重新启动后，客户端可能会收到一个TCP连接重置的信号。这是因为在服务端重新启动时，原先的TCP连接已经失效，新的服务端可能会拒绝或无法处理之前的连接。

Socket异常（Socket Exception）： 在很多编程语言中，如果服务端挂掉并重新启动，客户端可能会收到一个套接字异常。这通常是一种异常状态，表明之前的连接已经不再有效。

Timeout错误： 如果客户端在挂掉的服务端上发起了某个操作（例如发送数据）并且一段时间内未收到响应，客户端可能会触发超时错误。这是因为重新启动后的服务端可能需要一些时间来处理连接。
```
2、服务端进程短暂挂掉重启之后，客户端会收到什么通知吗
```
如果服务端进程短暂挂掉并重新启动，而且在此期间客户端并没有主动关闭连接，通常客户端可能不会收到明确的通知。TCP协议本身并没有提供直接的机制来通知客户端有关服务器进程重启的消息。客户端可能会察觉到连接中断或超时，但无法直接判断是因为服务端挂掉重启引起的。

以下是一些可能的影响和客户端可能观察到的行为：

连接重置（Connection Reset）： 如果服务端重新启动后，原先的TCP连接可能会被重置。客户端在尝试使用已重置的连接时可能会触发连接重置异常。

超时错误（Timeout）： 客户端在一段时间内未收到来自服务端的任何响应，可能会触发超时错误。这可能是因为服务端在重新启动期间无法响应请求。

连接丢失： 服务端重新启动可能导致连接丢失，从而触发一些连接丢失或连接超时的错误。

为了解决这些问题，客户端可以实现一些机制，例如：

心跳机制： 客户端定期向服务端发送心跳消息，以确保连接的活跃性。如果一段时间内未收到心跳响应，客户端可以认为连接已经断开，然后尝试重新连接。
