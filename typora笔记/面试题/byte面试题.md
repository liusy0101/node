...
隔葛性 (isolation）：一个事务所做的修改在最終提交以前，对其它事务是不可见的 （井发执行的事秀
之间不能相互影响）
持久性 (Durability)：一旦事务提交成功，对数据的修改是永久性的

### 1.41. Redis主从复制机制
1.请介绍一下Redis的主从复制机制
2.redis的副本集是如何保证数据一致性的？
3.主从数据同步流程

参考：https://segmentfault.com/a/1190000039060249

1、psync+offset，slave的offset在缓存中，则部分重同步，反之，则完全重同步。
![](typora-user-images/2023-11-12-17-47-04.png)
...
- 任务调度
- 任务监管
- 任务分类：一次性任务、定时任务、周期性任务


重点：调度器和执行器分开，定时扫描任务数据，将数据放入timewhell中

参考： https://juejin.cn/post/7056704703559630856


```text
系统架构：

采用微服务架构，分为任务调度服务、任务执行服务和存储服务。
使用负载均衡器确保服务的高可用性和容错性。
任务定义和调度策略：

任务包含类型、参数、触发条件和执行逻辑。
调度策略允许用户定义任务的执行频率和执行目标。
任务队列：

使用消息中间件（如 RabbitMQ）作为任务队列，确保任务的顺序执行和不丢失。
分布式存储：

使用分布式数据库（如 Cassandra）存储任务元数据和执行结果。
节点管理：

使用服务注册与发现工具（如 Consul）进行节点管理，确保任务可以分发到可用的执行节点。
任务执行器：

实现任务执行器，负责在执行节点上运行任务。
考虑任务的并发执行、重试机制和任务超时。
日志和监控：

记录任务执行日志到集中式日志存储（如 ELK Stack）。
集成监控系统（如 Prometheus）监控任务执行的性能和健康状态。
安全性：

实现身份验证和授权机制，确保只有授权用户可以管理任务。
数据传输使用加密协议（如 HTTPS）。
扩展性：

支持水平扩展，可以通过增加执行节点应对任务负载的增加。
实现自动化扩展机制，根据负载自动调整节点数量。
故障处理：

使用健康检查和自动重启来应对节点故障。
实现故障检测和恢复机制，确保系统在出现故障时能够快速自愈。
版本管理：

支持任务脚本或代码的版本管理，确保任务的执行环境是可控的。
用户接口：

提供 Web 界面和 RESTful API 接口，允许用户管理和监控任务。
```

### 1.43. redis中sortedset实现原理，使用场景
- 请描述下Sortedset的实现原理？
- 为什么进用SkipList来实现而不是其他数据结构来实现Sortedsort（例如哈希表、平衡树？）
- SortedSet应用场景

答案：
- 跳表+哈希表，查询/插入/删除操作可参考：https//www.jianshu.com/p/9d8296562806
- 原因：
  - skiplist和各种平衡树（如AVL、红黑树等）的元奏是有序排列的，而哈希表不是有序的。因此，在哈希表上只能做单个key的查找，不适宜做范国查找。所谓范国查找，指的是查找那些大小在指定的两个值之间的所有节点。
  - 在做范围查找的时候，平衡树比Skiplist操作要复东。在平衡树上，我们找到指定范国的小值之后，还需要以中序通历的顺序继续寻找其已不超过大值的节点。如果不对平街树进行一定的政造，这里的中序進历并不容易实现。而在skiplist上进行范国查找就非常简单，只需要在找到小值之后，对第1层链表进行若千步的遍历就可以实现。
  - 平衡树的插入和删除操作可能引1发子树的调整，逻辑复杂，而skiplist的插入和删除只需要修改相邻节点的指针。操作简单又快速。
  - 从内存占用上来说，skiplist比t平衡树更灵活一些。一般来说，平衡树每个节点包含2个指针（分别指向左右子树)，而skiplist每个节点包含的指针数目平均为1/（1-p)，具体取决于参数p的大小。如果像Redis里的实现一样，取p=1/4，那么平均每个节点包含1.33个指针。比平衡树更有优势，
  - 查找单个key，skiplist和平衡树的时间复杂度都为0(1og n)，大体相当；而哈希表在保持较低的哈希值冲突概率的前提下，查找时间复杂度接近0(1)，性能更高一些。所以我们平常使用的各种Map或dictionary结构，大都是基于哈希表实现的。
...
update/insert/delete使用当前读


### 1.46. 索引失效情况
以"%(表示任意0个或多个字符）”开头的LIKE语句；
OR语句前后没有同时使用家引;
数据类型出现隐式转化（如varchar不加单引1号的话可能会自动转换为imnt型）；
对于多列家引，必须满足 最左匹配原则/最左前级原则（最左优先，eg：多列家引1col1、col2和col3，则 家引生效的情形包括 col1或col1，col2或co1, col2, col3)；
如果MySQL估计全表扫描比紫引快，则不使用紫引（比如非常小的表)

### 1.47. 单机Redis是如何支持高并发访问
1.redis是基于内存的，内存的读写进度非常快（纯内存)：数据存在内存中，数据结构用HashMap.
HashMap的优势就是查找和操作的时间复杂度都是O(1)。
2.redis是单线程的，省去了很多上下文切换线程的时间 （避免线程切换和竟态消耗）。
3.redis使用IO多路复用技术 (IO mutiplexing，解决对多个1/0监听时,一个1/0阻塞影响其他10的问题），可以处理并发的连接（非阻塞10)

### 1.48. Linux文件系统：inode,inode存储了哪些东西，目录名，文件名存在哪里
...
### 1.49 深拷贝与浅拷贝
赋值（=），就是创建了对象的一个新的/用，修改其中任何一个变量都会影响到另一个。
淺拷贝copy.copy：创建一个新的对象，但它包含的是对原始对象中包含项的引用（如果用引用的方式修改其中一个对象，另外一个也会修改改变）
深拷贝：创建一个新的对象，并且递归的复制它所包含的对象(修改其中一个，另外一个不会改变){copy模块的deep.deepcopy0函数


### 1.50. TCP如何利用最大带宽
TCP（传输控制协议）是一种面向连接的协议，它在数据通信中起到可靠传输的作用。虽然TCP被设计用于在不可靠的网络环境中进行可靠数据传输，但在一定程度上，它也可以被调整以更好地利用网络的最大带宽。以下是一些方法，可以帮助TCP更好地利用可用的带宽：

- 窗口自适应：
  TCP使用滑动窗口机制来控制发送方可以发送多少个数据包而不等待确认。通过动态调整滑动窗口的大小，TCP可以更好地适应网络的状况，以提高带宽利用率。
  拥塞窗口和接收窗口的自适应调整是TCP的一项关键特性，它允许TCP动态地适应网络的条件，从而提高带宽利用率。
- 快速重传和快速恢复：
  TCP使用重传机制来确保可靠性。在发生数据包丢失时，TCP通常会等待一定的时间（重传超时）然后进行重传。为了更快地响应丢包，TCP引入了快速重传和快速恢复机制。如果接收方收到一个失序的报文段，它会立即向发送方发送重复的 ACK，而不等待重传超时。
  快速重传和快速恢复机制有助于更快地恢复数据流，减少网络带宽的浪费。
- TCP拥塞控制：
  TCP通过拥塞窗口和慢启动等机制来调整数据发送速率，以避免网络拥塞。拥塞窗口的调整可以防止网络的过度拥塞，确保带宽被有效地利用。
- 延迟确认：
  TCP的延迟确认机制允许接收方等待一段时间来发送确认，以期望在同一个确认中捎带多个ACK。这有助于减少ACK包的数量，提高网络的带宽利用率。
- TCP加速算法：
  一些TCP加速算法被设计用于更有效地利用网络带宽。这些算法可以优化数据传输的方式，以更好地适应高延迟、高带宽的网络。
- 带宽延迟积（BDP）：
  理解网络的带宽延迟积（BDP），即网络带宽与往返时延的乘积，有助于优化TCP参数以达到更好的带宽利用。


### 1.51. Redis 缓存穿透、雪崩、击穿如何解決？
**缓存穿透：**
指查询一个一定不存在的数据，如果从存储层查不到数据则不写入缓存，这将导致这个不存在的数据每次请求都要到DB 去查询，可能导致 DB 挂掉。
解决方案：
  1.查询返回的数据为空，仍把这个空结果进行缓存，但过期时间会比较短；
  2.布隆过滤器，将所有可能存在的数据哈希到一个足够大的 bitmap 中，一个一定不存在的数据会被这个 bitmap 拦截掉，从而避免了对 DB 的查询。
    缺点：多个hash函数的情况下，无法删除数据
**缓存击穿：**
对于设置了过期时间的 key，缓存在某个时间点过期的时候，恰好这时间点对这个 Key 有大量的并发请求过来，这些请求发现緩存过期一般都会从后端 DB 加载数据并回设到缓存，这个时候大并发的请求可能会瞬间把 DB 压垮。
解决方案：
  1.使用互斥锁：当緩存失效时，不立即去load db、先使用如 Redis 的 setnx 去没置一个互斥锁，当操作成功返回时再进行 load db 的操作并回设缓存，否则重试 get 緩存的方法。
  2.永远不过期：物理不过期，但逻辑过期 (后台异步线程去刷新）。
**缓存雪崩：**
设置缓存时采用了相同的过期时间，导致缓存在某一时刻同时失效，请求全部转发到 DB,DB瞬时压力过重雪崩。与缓存击穿的区别：雪崩是很多 key，击穿是某一个key 缓存。
解决方案：
  将缓存失效时间分散开，比如可以在原有的失效时间基础上增加一个随机值，比如 1-5 分钟随机，这样每一个緩存的过期时间的重复率就会降低，就很难引发集体失效的事件

### 1.52. Kafka如何保証不丟失消息？
复制因子：创建topic的时候指定复制因子大手1时，一个分区被分配到一个broker上，同时会在其他broker上维护一个分区副本；
isr列表：分区及其副本分别为leader和follower，leader对外提供读写服务，follower会向leader发送同步请求，拉取最新的数据，如果follower和leader的消息差距保持在一定范国之内，那么这个follower在isr列表内；当分区leader所在broker宕机，会从isr列表中选举一个follower作为新的leader提供服务
通过kafka的acks参数可以控制消息的发送行为，acks可选的值有0、1、all：当设置为0时，生产者消总发送成功即为成功，不关心是否写入到磁盘及后续操作；当设置为1时，消息发送到分区leader后写入磁盛即为成功；当设置为al时，消息发送到分区leader并写入碳盘后，同步给isr列表中的所有分区副本后即为成功


### 1.53. Redis 中是否有事务机制？事务支持回滚吗？
Redis 支持事务，和 mysql 的事务有很大区别，Redis 一旦开启事务其他线程也可以对这个 key进行操作很难像mysql 一样保证数据的原子性。
mysql  中一旦有线程对一行数据进行事务操作，其他线程对这一行数据是不能进行探作的，redis和mysql  正好相反，
Redis 的Watch 采用乐观锁机制控制，乐观锁主要是指当每次拿数据的时候都认为都别人不会更改，所以不会上锁，允许多个线程进行操作，这也是 Redis 不能保证数据原子性的原因所在。
redis 不存在事务回滚。
为什么redis 不采用事务回滚？
回滚的目的就是将mysql的行锁进行释放，而redis 不存在行锁，但是 redis 有取消事务，使用discard
会放弃事务块的所有命令。

### 1.54. RocketMQ如何保证消息不丢失
1. Producer端如何保证消息不丢失
采取send()同步 发消息，发送结果是同步感知的。
发送失败后可以重试，设置重试次数。默认3次。
集群部署，比如发送失败了的原因可能是当前Broker宕机了
2. Broker端如何保证消息不丢失
修改刷盘策略为同步刷盘。默认情况下是异步刷盘的，集群部署，主从模式，高可用。
3. Consumer端如何保证消息不丢失
完全消费正常后在进行手动ack确认

### 1.55.  什么是Rebalance，触发 Rebalance 的时机，Rebalance 过程
**什么是 Rebalance**
Rebalance 本质上是一种协议，规定了一个 Consumer Group 下的所有 consumer 如何达成一致，来分配订阅
Topic 的每个分区。
例如：某 Group 下有20个consumer 实例，它订阅了一个具有 100个 partition 的 Topic。正常情况下，kafka 会为每个 Consumer 平均的分配 5个分区。这个分配的过程就是 Rebalance.
**触发 Rebalance 的时机**
Rebalance 的触发条件有3个
1. 组成员个数发生变化。例如有新的consumer 实例加入该消费组或者离开组
2. 订阅的Topic 个数发生变化。
3. 订阅 Topic 的分区数发生变化。
Rebalance 发生时，Group下所有consumer 实例都会协调在一起共同参与，kafka 能够保证尽量达到最公平的分配。但是Rebalance 过程对 consumer group 会造成比较严重的影响。在 Rebalance
的过程中 consumer group 下的所有消费者实例都会停止工作，等待 Rebalance 过程完成。
**Rebalance过程**
...
SyncGroup: SyncGroup 请求的主要目的，就是让协调者把领导者制定的分配方案下发给各个組内成员。当所有成员都成功接收到分配方案后，消费者组进入到 Stable 状态，即开始正第的消费工作。


### 1.56. Kafka为什么这么快
分区，顺序写磁盘，0-copy 零拷贝、利用操作系统页緩存、磁盘顺序写
kafka零拷贝原理
分区、分段、建立案引
生产者、消费者批处理

### 1.57. 为什么RocketMQ不使用zookeeper作注册中心
1. 根据CAP理论，同时最多只能满足两个点，而200keeper满足的是CP，也就是说zookeeper并不能保证服务的可用性，zookeeper在进行选举的时候，整个选举的时间太长，期间堥个集群都处于不可用的状态，而这对于一个注册中心来说肯定是不能接受的，作为服务发现来说就应该是为可用性而设计。
2. 基于性能的考虑，NameServer本身的实现非常轻量，而且可以通过增加机器的方式水平扩展，增加集群的抗压能力，而zookeeper的写是不可扩展的，而zookeeper要解決这个问题只能通过划分领域，划分多个z0okeeper集群来解决，首先操作起来太复杂，其次这样还是叉违反了CAP中的A的设计，导致服务之间是不连通的。
3. 持久化的机制来带的问题，Zookeeper 的ZAB 协议对每一个写请求，会在每个 Zookeeper 节点上保持写一个事务日志，同时再加上定期的将内存数据镜像 (Snapshot)到磁盘来保证数据的一致性和持久性，而对于一个简单的服务发现的场景来说，这其实没有太大的必要，这个实现方紫太重了。而且本身存储的数据应该是高度定制化的。
...
事务消息就是MQ提供的类似XA的分布式事务能力，通过事务消息可以达到分布式事务的最終一致性，半事务消息就是MQ收到了生产者的消息，但是没有收到二次确认，不能投递的消息。
实现原理如下：
![](typora-user-images/2023-11-12-23-29-15.png)

1. 生产者先发送一条半事务消息到MQ
2. RocketMQ收到消息后返回ack強込
3. 生产者开始执行本地事务
4. 如果事务执行成功发送commit到MQ，失败发送rollback
5. 如果MQ长时间未收到生产者的二次确认commit或 者rollback,MQ对生产者发起消息回查
6. 生产者查询事务执行最终状态
7. 根据查询事务状态再次提交二次确认
最终，如果MQ收到二次确认commit，就可以把消息投递给消费者，反之如果是rollback，消息会保存下来井且在3天后被删除。

### 1.59. Kaika消息是采用Pull模式，还是Push模式？
Kafka最初考虑的问题是，customer应 该从brokes拉取消息还是brokers将消息推送到consumer，也就是pull还push。在这方面，Kafka遵循了一种大部分消息系统共同的传统的设计：producer将消息推送到broker， consumer从broker拉取消息。一些消息系统比如Scribe和Apache Flume采用了push模式，将消息推送到下游的consumer。这样做有好处也有坏处：由broker決定消息推送的迷率，对于不同消费速率的consumer就不太好处理了。消息系統都致力于让consumerl以最大的迭率最快速的消费消息，但不幸的是，push模式下，当broker推送的速率远大于consumer消费的速率时，consumer恐怕就崩溃了。最終Karka还是述取了传统的pul機式。Pull模式的另外一个好处是consurmer可以自主决定是否批量的从broker拉取数据。Push模式必须在 不知道下游consumer消费能力和消费策路的情况下决定是立即推送每条消息还是缓存之后批量推送。如果为了避免consumer艄溃而采用较低的推送速率，将可能导致一次只推送较少的消总而造成浪费。Pull模式下，consumer就可以根据自己的消费能力去决定这些策略。Pul有个缺点是，如果broker没有可供消费的消息，将导致consumer不断在循环中轮询，直到新消息到达。为了遊免这点，Kafka有个参数可以让consumer阻塞知道新消息到达(当然也可以阻塞直道消息的数量达到某个特定的量这样就可以批量发送)

### 1.60. katka高水位是什么，有什么作用
...
### 给定两个链表，求交叉节点
- 遍历两个链表，得到它们的长度。
- 使较长的链表先走 ∣m−n∣ 步，使两个链表的起始点距离交叉节点的距离相等。
- 同时遍历两个链表，找到交叉节点。
### 输出给定数字下一个比它大的数字
### 股票买卖问题
### 单链表每隔k个节点进行反转
### 判断一个IP是否是国内
### 用两个栈实现一个队列
### path sum，给定一个二叉树和数字n，判断二叉树上是否有一个路径上的数字之和等于给定的数字
### LUR缓存
### 最长无重复字串
### 赛马求最快n匹
### 求数组的最大区间和
...
### 单链表(奇数位升序，偶数位降序)的排序
### 把中文数字转成int数字
### 冒泡排序和快速排序的时间复杂度是啥，咋实现
### 区间合并
### 1-n数字字典序第k大
### 拆分字符串：一个单词列表、一个字符串，检查字符串是否可以由列表中的单词组成
### 接雨水
### 两数之和
### 找零钱问题
### 给定单链表，求离终点距离为 k 的节点
...
### 二叉树最大宽度
### 链表求和
给定一个链表 L1、“L2°每个元素是为10以内的正整数，链表表示一个数字，表头为高位。
求两个链表之和，以链表形式返回
### 数组0值筛选
### 旋转列表
### 最短子数组之和
...
12.转换后的双向链表节点顺序要与中序遍历的结果一致
3二叉树最左边的节点作为双向链表的头节点
### 二叉树的镜像
### 生成n阶螺旋数组
### 二叉树最近公共祖先
### 序列化和反序列化二叉树
将一棵二叉树序列化为字符串，并能字待串反序列为一棵树，
### 翻转单词
### 二叉树所有根到叶子路径组成的数字之和
### 找出旋转有序数组中的最小值
...
### 扑克牌的堆栈、队列操作
### 通配符匹配
### 给定数组求数组一个区间，该区问满足区间内最小数乘上区间内数字和结果最大
### 求空间曼哈顿距离最小点
### 输入一个数n，输入1~n的全排列
### 输出所有根节点到叶子节点的路径
### 搜索二维矩阵
### 字符串翻转
有n个字符串，你可以选择对其中一些字符串进行左石翻转;选择需要你符串整体为升序;
### 低空间复杂度的数组循环右移
将一个长为n的数组循环右移k位，要求时间复杂度O(n)，空间复杂度0(1)
### 矩阵中的最长递增路径
### 生成二叉树节点的邻居节点
### 多叉树深度优先遍历查找
### 验证二叉搜索树
