...
* 1. [后端](#)
	...
	* 1.8. [秒杀系统设计](#-1)
	* 1.9. [设计一个海量评论系统](#-1)
	* 1.10. [SQL中的Inner、Left、Right Join的区别](#SQLInnerLeftRightJoin)
	* 1.11. [推送去重系统设计](#-1)
	* 1.12. [设计类似微博timeline功能实现，考虑怎么设计支持热门](#timeline)
	* 1.13. [分布式缓存系统设计](#-1)
	* 1.14. [开放API设计](#API)
	* 1.15. [关系系统设计](#-1)
	* 1.16. [Redis过期策略和内存淘汰策略](#Redis-1)
	* 1.17. [乐观锁与悲观锁的区别](#-1)
	* 1.18. [小于n的最大数](#n)
	* 1.19. [Redis实现分布式锁](#Redis-1)
	* 1.20. [Cache设计](#Cache)
	* 1.21. [事务隔离级别](#-1)
	* 1.22. [分布式定时任务管理系统](#-1)
	* 1.23. [B和B+树的区别](#BB)
	* 1.24. [SELECT * FROM table WHERE a > 1and b=2：如何建立索引？](#SELECTFROMtableWHEREa1andb2)
	* 1.25. [区间重复值检测](#-1)
* 2. [算法](#-1)
...
###  1.10. <a name='SQLInnerLeftRightJoin'></a>SQL中的Inner、Left、Right Join的区别
...
###  1.12. <a name='timeline'></a>设计类似微博timeline功能实现，考虑怎么设计支持热门
...
###  1.16. <a name='Redis-1'></a>Redis过期策略和内存淘汰策略
**过期策略：**
...
###  1.19. <a name='Redis-1'></a>Redis实现分布式锁
...
###  1.24. <a name='SELECTFROMtableWHEREa1andb2'></a>SELECT * FROM table WHERE a > 1and b=2：如何建立索引？
主要考察联合索引、最左匹配几个知识点。
